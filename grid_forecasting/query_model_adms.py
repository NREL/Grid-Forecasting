...
def get_feeder_name(value):
    ...
    SELECT ?feeder_name WHERE {
                        ...
                        ORDER by ?feeder_name
    '''
    results = gridappsd_obj.query_data(query,timeout=300)
    results_obj = results['data']
    for b in results_obj['results']['bindings']:
        # print(b['feeder_name']['value'])
        # print(b['fid']['value'])
...
# SynchronousMachine - DistSyncMachine
    fidselect = """ VALUES ?fdrid {\"""" + value + """\"} """
    query = prefix+'''SELECT ?name ?bus (group_concat(distinct ?phs;separator="\\n") as ?phases) ?ratedS ?ratedU ?p ?q ?id ?fdrid WHERE {
...
#         print(b['eqtype']['value'] )
        ...
        if b['type']['value'] == 'VA':
            ...
            if b['eqtype']['value'] == u'SynchronousMachine':
                gen_power_map[name] = b['id']['value']
    ...
    print(mrid_types)
    print(trans_map)
    return result, name_map, node_name_map_va_power, node_name_map_pnv_voltage, gen_power_map, pec_map, load_power_map, line_map, \
...
def get_base_voltages(AllNodeNames,fidselect):
    lines = get_line_segements(fidselect)
    switches = get_switches(fidselect)
    trans1 = get_transformer_with_tanks(fidselect)
    trans2 = get_transformer_no_tanks(fidselect)
    data = get_basev_from(lines, switches, trans1, trans2)
...
if __name__ == '__main__':
    ...
    fid_select = '_49AD8E07-3BF9-A4E2-CB8F-C3722F837B62'  # ieee13nodeckt
    fid_select = '_AAE94E4A-Jeff'
    fid_select = '_AAE94E4A-Jeff-off-gen'

    # base_voltages = get_base_voltages('_AAE94E4A-2465-6F5E-37B1-3E72183A4E44')
    # print(base_voltages[0] * math.sqrt(3))

    # _88B39F80-3BE8-2BFC-50BA-4D75604810D2

    generators = get_generator(fid_select)
    print(generators)
    for g in generators:
        print("'"+g['bus'].upper()+".1', " + "'"+g['bus'].upper() + ".2', " + "'"+g['bus'].upper() + ".3', ")
    ...
    regs = get_regulator(fid_select_123)
    print(regs)

    switches = get_switches(fid_select_123)
    print(switches)
    ...
    result, name_map, node_name_map_va_power, node_name_map_pnv_voltage, gen_map, pec_map, load_power_map, line_map, \
           trans_map, switch_pos, cap_pos, tap_pos, load_voltage_map, line_voltage_map, trans_voltage_map = lookup_meas(fid_select)

    _generators = []
    _generators_dict = {}
    index = 0
    for generator in get_generator(fid_select):
        for phase_index in range(generator['numPhase']):
            phase_name = generator['phases'][phase_index]
            print(generator['bus'].upper(), phase_name)
            name = generator['bus'].upper() + '.' + lookup[phase_name]
            _generators.append(name)
            # index = AllNodeNames.index(name)
            index+=1
            _generators_dict[index] = {
                'name': generator['name'],
                'busname': name,
                'meas_mrid': gen_map[name],
    ...
    _generators_dict = collections.OrderedDict(sorted(_generators_dict.items()))
    print(json.dumps(_generators_dict, indent=2))
